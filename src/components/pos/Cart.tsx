        ...
        {step === "cart" ? (
          <>
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-start gap-3 p-3 rounded-xl bg-card border border-border/50">
                    <div className="flex-1 min-w-0">
                      ...
                      {item.note && (
                        <p className="text-[11px] text-muted-foreground/70 mt-0.5 leading-tight italic">📝 {item.note}</p>
                      )}
                      <p className="text-sm font-bold text-primary mt-1">{formatPrice(item.price * item.qty)}</p>
                    </div>
            ...
            <div className="border-t border-border p-4 safe-bottom">
              ...
              <Button onClick={() => setStep("checkout")} className="w-full h-12 rounded-xl text-base font-bold">
                Thanh toán
              </Button>
            </div>
          </>
